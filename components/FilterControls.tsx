...
const FilterControls: React.FC<FilterControlsProps> = ({ filters, setFilters, sortOption, setSortOption }) => {
  ...
  const FilterSelect: React.FC<{name: keyof FilterOptions, label: string, options: readonly string[]}> = ({name, label, options}) => (
     <div>
        ...
        <select
          ...
          className="block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-[#192F59] focus:border-[#192F59] sm:text-sm rounded-md shadow-sm bg-white text-gray-800"
        >
  ...
  return (
    <div className="bg-white p-4 rounded-lg shadow-sm mb-8 border border-gray-200">
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        ...
        <FilterSelect name="category" label="Category" options={PROJECT_CATEGORIES} />
        <FilterSelect name="status" label="Status" options={PROJECT_STATUSES} />
        <div>
