          ...
          <div className="p-8 space-y-6">
            ...
            <div className="space-y-4 border-b pb-6">
              ...
              <div>
                ...
                <input type="text" value={projectTitle} onChange={e => setProjectTitle(e.target.value)} className="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" required />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700">Description *</label>
                <textarea value={description} onChange={e => setDescription(e.target.value)} rows={4} className="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" required />
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <input type="url" value={githubLink} onChange={e => setGithubLink(e.target.value)} className="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" placeholder="GitHub Link (Optional)" />
                <input type="url" value={publicationLink} onChange={e => setPublicationLink(e.target.value)} className="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" placeholder="Publication/Demo Link (Optional)" />
              </div>
            ...
            <div className="space-y-4 border-b pb-6">
               <h3 className="text-lg font-semibold text-gray-700">Students *</h3>
                {students.map((student, index) => (
                    <div key={index} className="p-3 bg-gray-50 rounded-md border grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-3 items-end">
                       <input value={student.name} onChange={(e) => handleStudentChange(index, 'name', e.target.value)} placeholder="Student Name" className="w-full border-gray-300 rounded-md shadow-sm md:col-span-2" required />
                       <select value={student.year} onChange={(e) => handleStudentChange(index, 'year', e.target.value)} className="w-full border-gray-300 rounded-md shadow-sm"><option disabled>Year</option>{YEARS.map(y => <option key={y} value={y}>{y}</option>)}</select>
                       <select value={student.department} onChange={(e) => handleStudentChange(index, 'department', e.target.value)} className="w-full border-gray-300 rounded-md shadow-sm"><option disabled>Dept</option>{DEPARTMENTS.map(d => <option key={d} value={d}>{d}</option>)}</select>
                       <div className="flex items-center gap-2">
                         <select value={student.section} onChange={(e) => handleStudentChange(index, 'section', e.target.value)} className="w-full border-gray-300 rounded-md shadow-sm"><option disabled>Sec</option>{SECTIONS.map(s => <option key={s} value={s}>{s}</option>)}</select>
               ...
               <h3 className="text-lg font-semibold text-gray-700">Faculty Advisors (Optional)</h3>
                {faculty.map((f, index) => (
                    <div key={index} className="p-3 bg-gray-50 rounded-md border grid grid-cols-[1fr_1fr_auto] gap-3 items-end">
                       <input value={f.name} onChange={(e) => handleFacultyChange(index, 'name', e.target.value)} placeholder="Faculty Name" className="w-full border-gray-300 rounded-md shadow-sm" />
                       <select value={f.department} onChange={(e) => handleFacultyChange(index, 'department', e.target.value)} className="w-full border-gray-300 rounded-md shadow-sm"><option disabled>Dept</option>{DEPARTMENTS.map(d => <option key={d} value={d}>{d}</option>)}</select>
               ...
               <h3 className="text-lg font-semibold text-gray-700">Project Documents (Optional)</h3>
                {documents.map((doc, index) => (
                    <div key={index} className="p-3 bg-gray-50 rounded-md border grid grid-cols-1 md:grid-cols-[1fr_2fr_auto] gap-3 items-end">
                       <input value={doc.name} onChange={(e) => handleDocumentChange(index, 'name', e.target.value)} placeholder="Document Name (e.g., Abstract.pdf)" className="w-full border-gray-300 rounded-md shadow-sm" />
                       <input type="url" value={doc.url} onChange={(e) => handleDocumentChange(index, 'url', e.target.value)} placeholder="Document URL" className="w-full border-gray-300 rounded-md shadow-sm" />
            ...
            <div className="space-y-4 border-t pt-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <select name="year" value={year} onChange={e => setYear(e.target.value)} className="mt-1 block w-full border-gray-300 rounded-md shadow-sm"><option disabled>Year</option>{YEARS.map(y => <option key={y} value={y}>{y}</option>)}</select>
                    <select name="department" value={department} onChange={e => setDepartment(e.target.value)} className="mt-1 block w-full border-gray-300 rounded-md shadow-sm"><option disabled>Department</option>{DEPARTMENTS.map(d => <option key={d} value={d}>{d}</option>)}</select>
                    <select name="section" value={section} onChange={e => setSection(e.target.value)} className="mt-1 block w-full border-gray-300 rounded-md shadow-sm"><option disabled>Section</option>{SECTIONS.map(s => <option key={s} value={s}>{s}</option>)}</select>
                </div>
            </div>
